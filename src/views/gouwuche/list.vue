...
<el-option v-for="m in shangpinfenleiList" :value="m.id" :label="m.fenleimingcheng"></el-option>
</el-select>
                ...
                </el-form-item>                <el-form-item label="小计">
                
                    <el-input type="text" style="width:80px;" v-model="search.xiaoji_start"></el-input>
-<el-input type="text" style="width:80px;" v-model="search.xiaoji_end"></el-input>
        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row>
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.shangpinbianhao  }}                </template>
            </el-table-column>
                        <el-table-column label="分类"width="80">
                <template slot-scope="{row}">
                     <e-select-view module="shangpinfenlei" :value="row.fenlei" select="id" show="fenleimingcheng"></e-select-view>                </template>
            </el-table-column>
                        <el-table-column label="商品名称">
                <template slot-scope="{row}">
                    {{  row.shangpinmingcheng  }}                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    ...
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/gouwucheupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.gouwuche.list , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.gouwuche.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
