        ...
        <el-card class="box-card">
            ...
            <el-table border :data="list" style="width: 100%" highlight-current-row>
                ...
                <el-table-column label="配送员体温" width="80">
                    <template slot-scope="{row}">
                        {{ row.peisongyuantiwen }}
                    </template>
                </el-table-column>
                <el-table-column label="外卖图片" width="100">
                    <template slot-scope="{row}">
                        <e-img :src="row.waimaitupian" style="max-width:120px"/>
                    </template>
                </el-table-column>
                <el-table-column label="配送照片" width="100">
                    <template slot-scope="{row}">
                        <e-img :src="row.peisongzhaopian" style="max-width:120px"/>
                    </template>
                </el-table-column>
                    ...
                    <template slot-scope="{row}">
                        <el-button-group>
                            ...
                            <el-tooltip   content="详情" placement="top">
                                <el-button @click="$goto({path:'/admin/dingdanquxiaodetail',query:{id:row.id } })"
                                           icon="el-icon-info" type="info" size="mini"></el-button>
                            </el-tooltip>
                            <el-tooltip  v-if="$session.cx == '管理员'"      content="编辑" placement="top">
                                <el-button icon="el-icon-edit"
                                           @click="$goto({path:'/admin/dingdanquxiaoupdt',query:{id:row.id } })"
                                           type="warning" size="mini"></el-button>
                            </el-tooltip>
                            <el-tooltip v-if="$session.cx == '管理员'"     content="删除" placement="top">
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                this.$post(api.dingdanquxiao.list, filter).then(res => {
                    this.loading = false;
