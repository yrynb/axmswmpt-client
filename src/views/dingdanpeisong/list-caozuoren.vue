<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                ...
                </el-form-item>                <el-form-item label="收餐人">
                
                    <el-input v-model="search.shoucanren"></el-input>
                    
                </el-form-item>                <el-form-item label="存餐位置">
        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row>
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.xingming  }}                </template>
                        ...
                        <el-table-column label="配送员体温"width="80">
                <template slot-scope="{row}">
                    {{  row.peisongyuantiwen  }}                </template>
            </el-table-column>
                        <el-table-column label="外卖图片"width="100">
                <template slot-scope="{row}">
                    <e-img :src="row.waimaitupian" style="max-width:120px" />                </template>
            </el-table-column>
                        <el-table-column label="配送照片"width="100">
                <template slot-scope="{row}">
                    <e-img :src="row.peisongzhaopian" style="max-width:120px" />                </template>
            </el-table-column>
                        <el-table-column label="操作人"width="80">
                <template slot-scope="{row}">
                    {{  row.caozuoren  }}                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                    <el-button @click="$goto('/admin/dingdanquxiaoadd?id='+row.id)" type="success" icon="el-icon-plus" size="mini">
                                订单取消添加
                            </el-button>
                            <el-button type="info" @click="'/admin/dingdanquxiao?dingdanpeisongid='+row.id" icon="el-icon-search" size="mini">
                                订单取消查询
                            </el-button>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/dingdanpeisongupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.dingdanpeisong.listcaozuoren , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.dingdanpeisong.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
