<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                ...
                </el-form-item>                <el-form-item label="配送评级">
                
                    <el-select v-model="search.peisongpingji" ><el-option label="请选择" value=""></el-option><el-option label="好评" value="好评"></el-option>
...
<el-option label="差评" value="差评"></el-option>
                    ...
                    <el-input v-model="search.pingjia"></el-input>
            ...
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.dingdanjine  }}                </template>
                        ...
                        <el-table-column label="配送评级"width="80">
                <template slot-scope="{row}">
                    {{  row.peisongpingji  }}                </template>
            </el-table-column>
                        <el-table-column label="评价">
                <template slot-scope="{row}">
                    {{  row.pingjia  }}                </template>
            </el-table-column>
                        <el-table-column label="评价人"width="80">
                <template slot-scope="{row}">
                    {{  row.pingjiaren  }}                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                        ...
                        <el-button @click="$goto({path:'/admin/dingdanqianshoudetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/dingdanqianshouupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.dingdanqianshou.listpingjiaren , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.dingdanqianshou.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
