<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                    ...
                    <el-select v-model="search.zhuangtai" ><el-option label="请选择" value=""></el-option><el-option label="体温正常" value="体温正常"></el-option>
            ...
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.nianling  }}                </template>
                        ...
                        <el-table-column label="检测体温"width="80">
                <template slot-scope="{row}">
                    {{  row.jiancetiwen  }}                </template>
            </el-table-column>
                ...
                <template slot-scope="{row}">
                    {{  row.beizhu  }}                </template>
                        ...
                        <el-table-column label="检测人"width="80">
                <template slot-scope="{row}">
                    {{  row.jianceren  }}                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                        ...
                        <el-button @click="$goto({path:'/admin/tiwenjiancedetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/tiwenjianceupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.tiwenjiance.listjianceren , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.tiwenjiance.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
